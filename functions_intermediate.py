...
def iterateDict( yourlist ):
    for element in yourlist:
        print("first_name - ", element["first_name"], ", ", "last_name - ", element["last_name"])
...
def printInfo(some_dict):
    ...
    for x in range(0,len(keylist),1):
        print(f"{len(some_dict[keylist[x]])} {keylist[x]}")
        for y in range(0,len(some_dict[keylist[x]]),1):
            print(some_dict[keylist[x]][y])
